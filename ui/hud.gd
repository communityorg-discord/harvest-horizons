extends CanvasLayer
## Cozy game HUD. Built procedurally so the .tscn stays a one-line stub.
## Layout matches concept art: player card top-left, top-bar centre,
## minimap top-right, hotbar bottom-centre, side menu bottom-right,
## quest log right-side.

# ────────────────────────────────────────────────────────────────────────────
# Theme

const PARCHMENT       := Color(0.96, 0.92, 0.82, 0.95)
const PARCHMENT_DEEP  := Color(0.88, 0.82, 0.66, 0.95)
const DARK_PANEL      := Color(0.18, 0.14, 0.10, 0.88)
const INK             := Color(0.22, 0.16, 0.10)
const INK_SOFT        := Color(0.45, 0.35, 0.22)
const BORDER          := Color(0.55, 0.40, 0.25)
const GOLD            := Color(0.78, 0.62, 0.28)
const LEAF            := Color(0.40, 0.55, 0.22)
const BERRY           := Color(0.72, 0.32, 0.22)
const HP_GREEN        := Color(0.40, 0.74, 0.32)
const EN_BLUE         := Color(0.32, 0.62, 0.86)

const HOTBAR_TOOLS := [
	{"name": "Hoe",     "label": "H"},
	{"name": "Water",   "label": "W"},
	{"name": "Axe",     "label": "A"},
	{"name": "Pickaxe", "label": "P"},
	{"name": "Sword",   "label": "S"},
	{"name": "Seeds",   "label": "*"},
]

const SIDE_MENU := ["Inventory", "Skills", "Journal", "People", "Map"]

# ────────────────────────────────────────────────────────────────────────────
# Player stats (live here for now; will move to a PlayerState autoload later)

var hp: int = 100
var hp_max: int = 100
var energy: int = 80
var energy_max: int = 100
var level: int = 1
var hotbar_index: int = 0

# ────────────────────────────────────────────────────────────────────────────
# Cached node refs (filled during build)

var _money_label: Label
var _weather_label: Label
var _date_label: Label
var _time_label: Label
var _hp_bar: ProgressBar
var _hp_text: Label
var _en_bar: ProgressBar
var _en_text: Label
var _hotbar_slots: Array[PanelContainer] = []
var _hotbar_count_labels: Array[Label] = []
var _rank_label: Label
var _inventory_label: Label

# ────────────────────────────────────────────────────────────────────────────

func _ready() -> void:
	_build_player_card()
	_build_top_bar()
	_build_minimap()
	_build_quest_panel()
	_build_hotbar()
	_build_inventory_strip()
	_build_side_menu()
	_build_chat_log()

	GameState.money_changed.connect(_on_money)
	GameState.time_changed.connect(_on_time)
	GameState.day_changed.connect(_on_day)
	GameState.weather_changed.connect(_on_weather)
	GameState.rank_changed.connect(_on_rank)
	GameState.inventory_changed.connect(_on_inventory)
	_refresh_all()
	_select_hotbar(0)

func _input(event: InputEvent) -> void:
	if event.is_action_pressed("hotbar_1"): _select_hotbar(0)
	elif event.is_action_pressed("hotbar_2"): _select_hotbar(1)
	elif event.is_action_pressed("hotbar_3"): _select_hotbar(2)
	elif event.is_action_pressed("hotbar_4"): _select_hotbar(3)
	elif event.is_action_pressed("hotbar_5"): _select_hotbar(4)
	elif event.is_action_pressed("hotbar_6"): _select_hotbar(5)

# ────────────────────────────────────────────────────────────────────────────
# Stylebox helpers

func _parchment_box(corner: int = 10, border_w: int = 2) -> StyleBoxFlat:
	var sb := StyleBoxFlat.new()
	sb.bg_color = PARCHMENT
	sb.set_corner_radius_all(corner)
	sb.set_content_margin_all(10)
	sb.border_width_left = border_w
	sb.border_width_right = border_w
	sb.border_width_top = border_w
	sb.border_width_bottom = border_w
	sb.border_color = BORDER
	sb.shadow_color = Color(0, 0, 0, 0.25)
	sb.shadow_size = 6
	sb.shadow_offset = Vector2(0, 3)
	return sb

func _dark_box(corner: int = 10) -> StyleBoxFlat:
	var sb := StyleBoxFlat.new()
	sb.bg_color = DARK_PANEL
	sb.set_corner_radius_all(corner)
	sb.set_content_margin_all(10)
	sb.border_width_left = 2
	sb.border_width_right = 2
	sb.border_width_top = 2
	sb.border_width_bottom = 2
	sb.border_color = GOLD
	return sb

func _label(text: String, size: int, color: Color, align_center: bool = false) -> Label:
	var l := Label.new()
	l.text = text
	l.add_theme_font_size_override("font_size", size)
	l.add_theme_color_override("font_color", color)
	if align_center:
		l.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	return l

# ────────────────────────────────────────────────────────────────────────────
# Top-left: player card with avatar, HP, Energy, level

func _build_player_card() -> void:
	var panel := PanelContainer.new()
	panel.add_theme_stylebox_override("panel", _dark_box(12))
	panel.set_anchors_preset(Control.PRESET_TOP_LEFT)
	panel.offset_left = 16
	panel.offset_top = 16
	panel.offset_right = 16 + 280
	panel.offset_bottom = 16 + 92
	add_child(panel)

	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 12)
	panel.add_child(row)

	# Avatar circle
	var avatar := Panel.new()
	avatar.custom_minimum_size = Vector2(64, 64)
	var av_sb := StyleBoxFlat.new()
	av_sb.bg_color = LEAF
	av_sb.set_corner_radius_all(32)
	av_sb.border_width_left = 3
	av_sb.border_width_right = 3
	av_sb.border_width_top = 3
	av_sb.border_width_bottom = 3
	av_sb.border_color = GOLD
	avatar.add_theme_stylebox_override("panel", av_sb)
	row.add_child(avatar)
	var av_label := _label(str(level), 24, Color.WHITE, true)
	av_label.set_anchors_preset(Control.PRESET_FULL_RECT)
	av_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	avatar.add_child(av_label)
	_rank_label = av_label

	# Bars stack
	var bars := VBoxContainer.new()
	bars.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	bars.add_theme_constant_override("separation", 6)
	row.add_child(bars)

	var hp_pair := _bar_with_label("HP", HP_GREEN)
	bars.add_child(hp_pair[0])
	_hp_bar = hp_pair[1]
	_hp_text = hp_pair[2]
	_hp_bar.max_value = hp_max
	_hp_bar.value = hp
	_hp_text.text = "%d / %d" % [hp, hp_max]

	var en_pair := _bar_with_label("EN", EN_BLUE)
	bars.add_child(en_pair[0])
	_en_bar = en_pair[1]
	_en_text = en_pair[2]
	_en_bar.max_value = energy_max
	_en_bar.value = energy
	_en_text.text = "%d / %d" % [energy, energy_max]

func _bar_with_label(prefix: String, fill_color: Color) -> Array:
	var wrap := Control.new()
	wrap.custom_minimum_size = Vector2(0, 22)
	var bar := ProgressBar.new()
	bar.set_anchors_preset(Control.PRESET_FULL_RECT)
	bar.show_percentage = false
	var bg := StyleBoxFlat.new()
	bg.bg_color = Color(0, 0, 0, 0.5)
	bg.set_corner_radius_all(4)
	bg.border_width_left = 1
	bg.border_width_right = 1
	bg.border_width_top = 1
	bg.border_width_bottom = 1
	bg.border_color = Color(1, 1, 1, 0.2)
	var fill := StyleBoxFlat.new()
	fill.bg_color = fill_color
	fill.set_corner_radius_all(4)
	bar.add_theme_stylebox_override("background", bg)
	bar.add_theme_stylebox_override("fill", fill)
	wrap.add_child(bar)

	var l := Label.new()
	l.set_anchors_preset(Control.PRESET_FULL_RECT)
	l.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	l.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	l.add_theme_font_size_override("font_size", 12)
	l.add_theme_color_override("font_color", Color.WHITE)
	l.add_theme_color_override("font_outline_color", Color.BLACK)
	l.add_theme_constant_override("outline_size", 2)
	l.text = prefix
	wrap.add_child(l)
	return [wrap, bar, l]

# ────────────────────────────────────────────────────────────────────────────
# Top centre: money / weather / date / time strip

func _build_top_bar() -> void:
	var panel := PanelContainer.new()
	panel.add_theme_stylebox_override("panel", _dark_box(12))
	panel.set_anchors_preset(Control.PRESET_CENTER_TOP)
	panel.offset_left = -300
	panel.offset_right = 300
	panel.offset_top = 16
	panel.offset_bottom = 16 + 76
	add_child(panel)

	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 18)
	row.alignment = BoxContainer.ALIGNMENT_CENTER
	panel.add_child(row)

	_money_label = _add_top_stat(row, "MONEY", "0g", GOLD)
	_add_divider(row)
	_weather_label = _add_top_stat(row, "WEATHER", "Sunny", Color.WHITE)
	_add_divider(row)
	_date_label = _add_top_stat(row, "DATE", "Spring 1, Y1", Color.WHITE)
	_add_divider(row)
	_time_label = _add_top_stat(row, "TIME", "09:00 AM", Color.WHITE)

func _add_top_stat(parent: Node, title: String, value: String, value_color: Color) -> Label:
	var col := VBoxContainer.new()
	col.add_theme_constant_override("separation", 2)
	col.alignment = BoxContainer.ALIGNMENT_CENTER
	parent.add_child(col)
	var t := _label(title, 11, GOLD, true)
	col.add_child(t)
	var v := _label(value, 22, value_color, true)
	col.add_child(v)
	return v

func _add_divider(parent: Node) -> void:
	var d := ColorRect.new()
	d.color = Color(1, 1, 1, 0.15)
	d.custom_minimum_size = Vector2(1, 50)
	parent.add_child(d)

# ────────────────────────────────────────────────────────────────────────────
# Top-right: minimap stub

func _build_minimap() -> void:
	var wrap := PanelContainer.new()
	wrap.set_anchors_preset(Control.PRESET_TOP_RIGHT)
	wrap.offset_left = -180
	wrap.offset_top = 16
	wrap.offset_right = -16
	wrap.offset_bottom = 16 + 164
	wrap.add_theme_stylebox_override("panel", _dark_box(12))
	add_child(wrap)

	var col := VBoxContainer.new()
	col.add_theme_constant_override("separation", 4)
	wrap.add_child(col)

	col.add_child(_label("GREENFIELD", 10, GOLD, true))

	var map := Panel.new()
	map.custom_minimum_size = Vector2(140, 100)
	var mb := StyleBoxFlat.new()
	mb.bg_color = Color(0.20, 0.32, 0.18)
	mb.set_corner_radius_all(60)
	mb.border_width_left = 2
	mb.border_width_right = 2
	mb.border_width_top = 2
	mb.border_width_bottom = 2
	mb.border_color = GOLD
	map.add_theme_stylebox_override("panel", mb)
	col.add_child(map)

	# Player dot in middle
	var dot := Panel.new()
	dot.custom_minimum_size = Vector2(8, 8)
	dot.set_anchors_preset(Control.PRESET_CENTER)
	dot.offset_left = -4; dot.offset_top = -4
	dot.offset_right = 4; dot.offset_bottom = 4
	var dsb := StyleBoxFlat.new()
	dsb.bg_color = BERRY
	dsb.set_corner_radius_all(4)
	dot.add_theme_stylebox_override("panel", dsb)
	map.add_child(dot)

# ────────────────────────────────────────────────────────────────────────────
# Right side: quest panel

func _build_quest_panel() -> void:
	var panel := PanelContainer.new()
	panel.set_anchors_preset(Control.PRESET_CENTER_RIGHT)
	panel.offset_left = -276
	panel.offset_right = -16
	panel.offset_top = -100
	panel.offset_bottom = 100
	panel.add_theme_stylebox_override("panel", _dark_box(12))
	add_child(panel)

	var col := VBoxContainer.new()
	col.add_theme_constant_override("separation", 8)
	panel.add_child(col)

	col.add_child(_label("ACTIVE QUESTS", 12, GOLD, true))

	var sep := ColorRect.new()
	sep.color = Color(1, 1, 1, 0.2)
	sep.custom_minimum_size = Vector2(0, 1)
	col.add_child(sep)

	_add_quest(col, "★", "A Letter from the Wizard", "Read the letter at your farm.", GOLD)
	_add_quest(col, "🌱", "First Harvest", "Plant and harvest your first crop.", LEAF)
	_add_quest(col, "🔨", "A Smith in Need", "Bring 5 stone and 5 wood to Thomas.", BERRY)

func _add_quest(parent: Node, icon: String, title: String, desc: String, accent: Color) -> void:
	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 8)
	parent.add_child(row)
	var icn := _label(icon, 20, accent)
	row.add_child(icn)
	var col := VBoxContainer.new()
	col.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_child(col)
	var t := _label(title, 14, Color.WHITE)
	col.add_child(t)
	var d := _label(desc, 11, Color(0.85, 0.80, 0.68))
	d.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	col.add_child(d)

# ────────────────────────────────────────────────────────────────────────────
# Bottom centre: hotbar

func _build_hotbar() -> void:
	var panel := PanelContainer.new()
	panel.set_anchors_preset(Control.PRESET_CENTER_BOTTOM)
	var slot_w: int = 60
	var total_w: int = HOTBAR_TOOLS.size() * slot_w + (HOTBAR_TOOLS.size() - 1) * 6 + 24
	panel.offset_left = -total_w / 2
	panel.offset_right = total_w / 2
	panel.offset_top = -86
	panel.offset_bottom = -16
	panel.add_theme_stylebox_override("panel", _dark_box(12))
	add_child(panel)

	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 6)
	panel.add_child(row)

	for i in range(HOTBAR_TOOLS.size()):
		var slot := PanelContainer.new()
		slot.custom_minimum_size = Vector2(slot_w, 60)
		slot.add_theme_stylebox_override("panel", _slot_box(false))
		row.add_child(slot)
		_hotbar_slots.append(slot)

		var inner := VBoxContainer.new()
		inner.alignment = BoxContainer.ALIGNMENT_CENTER
		slot.add_child(inner)

		var icon := _label(HOTBAR_TOOLS[i].label, 22, INK, true)
		inner.add_child(icon)

		var name := _label(HOTBAR_TOOLS[i].name, 10, INK_SOFT, true)
		inner.add_child(name)

		var num := Label.new()
		num.text = str(i + 1)
		num.add_theme_font_size_override("font_size", 10)
		num.add_theme_color_override("font_color", BERRY)
		num.position = Vector2(4, 2)
		slot.add_child(num)

		# Bottom-right item count badge (only meaningful for seed slot for now).
		var count := Label.new()
		count.add_theme_font_size_override("font_size", 11)
		count.add_theme_color_override("font_color", INK)
		count.add_theme_color_override("font_outline_color", PARCHMENT)
		count.add_theme_constant_override("outline_size", 2)
		count.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT)
		count.offset_left = -22
		count.offset_top = -16
		count.offset_right = -4
		count.offset_bottom = -2
		count.text = ""
		slot.add_child(count)
		_hotbar_count_labels.append(count)

func _build_inventory_strip() -> void:
	var panel := PanelContainer.new()
	panel.set_anchors_preset(Control.PRESET_CENTER_BOTTOM)
	panel.offset_left = -180
	panel.offset_right = 180
	panel.offset_top = -132
	panel.offset_bottom = -94
	panel.add_theme_stylebox_override("panel", _dark_box(8))
	add_child(panel)

	_inventory_label = Label.new()
	_inventory_label.add_theme_font_size_override("font_size", 13)
	_inventory_label.add_theme_color_override("font_color", Color.WHITE)
	_inventory_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	panel.add_child(_inventory_label)
	_refresh_inventory_strip()

func _refresh_inventory_strip() -> void:
	if _inventory_label == null:
		return
	var parts: Array[String] = []
	for k in ["parsnip_seeds", "parsnip", "wood", "stone"]:
		var n: int = GameState.item_count(k)
		if n > 0:
			parts.append("%s ×%d" % [_pretty(k), n])
	_inventory_label.text = "  ·  ".join(parts) if parts.size() > 0 else "(empty)"

func _pretty(item_id: String) -> String:
	return item_id.capitalize().replace("_", " ")

func _slot_box(selected: bool) -> StyleBoxFlat:
	var sb := StyleBoxFlat.new()
	sb.bg_color = PARCHMENT if not selected else Color(1.0, 0.94, 0.72)
	sb.set_corner_radius_all(8)
	sb.set_content_margin_all(4)
	var bw: int = 3 if selected else 2
	sb.border_width_left = bw
	sb.border_width_right = bw
	sb.border_width_top = bw
	sb.border_width_bottom = bw
	sb.border_color = GOLD if selected else BORDER
	return sb

func _select_hotbar(i: int) -> void:
	if i < 0 or i >= _hotbar_slots.size(): return
	hotbar_index = i
	GameState.set_current_tool(i)
	for j in range(_hotbar_slots.size()):
		_hotbar_slots[j].add_theme_stylebox_override("panel", _slot_box(j == i))

# ────────────────────────────────────────────────────────────────────────────
# Bottom right: side menu

func _build_side_menu() -> void:
	var panel := PanelContainer.new()
	panel.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT)
	var btn_w: int = 78
	var total_w: int = SIDE_MENU.size() * btn_w + (SIDE_MENU.size() - 1) * 6 + 24
	panel.offset_left = -total_w
	panel.offset_right = -16
	panel.offset_top = -86
	panel.offset_bottom = -16
	panel.add_theme_stylebox_override("panel", _dark_box(12))
	add_child(panel)

	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 6)
	panel.add_child(row)

	for label in SIDE_MENU:
		var btn := Button.new()
		btn.text = label
		btn.custom_minimum_size = Vector2(78, 60)
		btn.add_theme_font_size_override("font_size", 12)
		btn.add_theme_color_override("font_color", INK)
		var nb := _slot_box(false)
		btn.add_theme_stylebox_override("normal", nb)
		btn.add_theme_stylebox_override("hover", _slot_box(true))
		btn.add_theme_stylebox_override("pressed", _slot_box(true))
		row.add_child(btn)

# ────────────────────────────────────────────────────────────────────────────
# Bottom left: chat / event log

func _build_chat_log() -> void:
	var panel := PanelContainer.new()
	panel.set_anchors_preset(Control.PRESET_BOTTOM_LEFT)
	panel.offset_left = 16
	panel.offset_right = 16 + 320
	panel.offset_top = -150
	panel.offset_bottom = -16
	panel.add_theme_stylebox_override("panel", _dark_box(12))
	add_child(panel)

	var col := VBoxContainer.new()
	col.add_theme_constant_override("separation", 4)
	panel.add_child(col)
	col.add_child(_label("EVENT LOG", 11, GOLD))

	var sep := ColorRect.new()
	sep.color = Color(1, 1, 1, 0.15)
	sep.custom_minimum_size = Vector2(0, 1)
	col.add_child(sep)

	_add_log(col, "Wizard:", "Welcome to Greenfield Valley.", GOLD)
	_add_log(col, "Mayor:", "The Harvest Festival is in 12 days!", LEAF)
	_add_log(col, "Tip:", "Press 1-6 to switch tools.", Color(0.8, 0.75, 0.6))

func _add_log(parent: Node, who: String, msg: String, who_color: Color) -> void:
	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 6)
	parent.add_child(row)
	var w := _label(who, 12, who_color)
	row.add_child(w)
	var m := _label(msg, 12, Color(0.92, 0.88, 0.76))
	m.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_child(m)

# ────────────────────────────────────────────────────────────────────────────
# Signal handlers

func _on_money(amount: int) -> void:
	_money_label.text = "%dg" % amount

func _on_time(hour: int, minute: int) -> void:
	var ampm: String = "AM" if hour < 12 else "PM"
	var h12: int = hour % 12
	if h12 == 0:
		h12 = 12
	_time_label.text = "%02d:%02d %s" % [h12, minute, ampm]

func _on_day(day: int, _month: int, year: int) -> void:
	_date_label.text = "%s %d, Y%d" % [GameState.get_season(), day, year]

func _on_weather(_w: int) -> void:
	_weather_label.text = GameState.weather_name()

func _on_rank(_r: int) -> void:
	pass # Rank-up popup will go here in Phase 3a.

func _on_inventory(item_id: String, count: int) -> void:
	_refresh_inventory_strip()
	# Update count badge on hotbar slot if this item maps to a slot.
	if item_id == "parsnip_seeds" and _hotbar_count_labels.size() > 5:
		_hotbar_count_labels[5].text = str(count) if count > 0 else ""

func _refresh_all() -> void:
	_on_money(GameState.money)
	_on_time(int(GameState.hour), int(GameState.minute))
	_on_day(GameState.day, GameState.month, GameState.year)
	_on_weather(GameState.weather)
	_refresh_inventory_strip()
	if _hotbar_count_labels.size() > 5:
		var seeds: int = GameState.item_count("parsnip_seeds")
		_hotbar_count_labels[5].text = str(seeds) if seeds > 0 else ""
